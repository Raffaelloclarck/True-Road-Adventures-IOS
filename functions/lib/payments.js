...
exports.stripeWebhook = exports.confirmCashPayment = exports.captureRidePayment = exports.listPaymentMethods = exports.createStripeSetupIntent = exports.createStripePaymentIntent = void 0;
...
function toStripeAmount(amountSrd) {
 ...
 */
exports.createStripePaymentIntent = (0, https_1.onCall)({ secrets: [stripeSecretKey], region: "us-central1" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    const { rideId, amount, currency } = request.data;
    if (amount == null || amount <= 0) {
        throw new https_1.HttpsError("invalid-argument", "Positive amount is required.");
    }
    ...
    if (!rideSnap.exists)
        throw new https_1.HttpsError("not-found", "Ride not found.");
    const ride = rideSnap.data();
    if (ride.customerId !== uid) {
        throw new https_1.HttpsError("permission-denied", "Only the rider can authorize payment.");
    }
    const existingIntentId = ride.paymentIntentId;
    if (existingIntentId) {
...
exports.createStripeSetupIntent = (0, https_1.onCall)({ secrets: [stripeSecretKey], region: "us-central1" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    const stripe = stripeClient(stripeSecretKey.value());
...
exports.listPaymentMethods = (0, https_1.onCall)({ secrets: [stripeSecretKey], region: "us-central1" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
 ...
 */
exports.captureRidePayment = (0, https_1.onCall)({ secrets: [stripeSecretKey], region: "us-central1" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    const { rideId, amount } = request.data;
    if (!rideId || amount == null) {
        throw new https_1.HttpsError("invalid-argument", "rideId and amount are required.");
    }
    const rideRef = admin.firestore().collection("rides").doc(rideId);
    const rideSnap = await rideRef.get();
    if (!rideSnap.exists)
    ...
    const ride = rideSnap.data();
    if (ride.customerId !== uid) {
        throw new https_1.HttpsError("permission-denied", "Only the rider can capture payment.");
    }
    const intentId = ride.paymentIntentId;
    if (!intentId) {
        throw new https_1.HttpsError("failed-precondition", "No payment authorization on this ride.");
    }
    const stripe = stripeClient(stripeSecretKey.value());
    ...
    if (intent.status === "requires_capture") {
        ...
        throw new https_1.HttpsError("failed-precondition", `PaymentIntent status: ${intent.status}`);
    }
    await rideRef.update({
        paymentStatus: "PAID",
        updatedAt: Date.now(),
...
exports.confirmCashPayment = (0, https_1.onCall)({ region: "us-central1" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "Sign in required.");
    const { rideId } = request.data;
    if (!rideId)
        throw new https_1.HttpsError("invalid-argument", "rideId is required.");
    const rideRef = admin.firestore().collection("rides").doc(rideId);
    const rideSnap = await rideRef.get();
    if (!rideSnap.exists)
        throw new https_1.HttpsError("not-found", "Ride not found.");
    ...
    if (ride.driverId !== uid) {
        throw new https_1.HttpsError("permission-denied", "Only the assigned driver can confirm cash.");
    }
    if (ride.paymentMethod !== "CASH") {
        throw new https_1.HttpsError("failed-precondition", "This ride is not a cash ride.");
    }
    if (ride.status !== "COMPLETED") {
        throw new https_1.HttpsError("failed-precondition", "Ride must be completed first.");
    }
    await rideRef.update({
        paymentStatus: "CASH",
        updatedAt: Date.now(),
...
exports.stripeWebhook = (0, https_1.onRequest)({ secrets: [stripeSecretKey, stripeWebhookSecret], region: "us-central1" }, async (req, res) => {
    if (req.method !== "POST") {
